...
int32_t ecm_ratelimit_check(struct s_reader *reader, ECM_REQUEST *er, int32_t reader_mode)
{
	...
	int32_t foundspace = -1, h, maxslots = MAXECMRATELIMIT; // init slots to ncam global maximums
	struct ecmrl rl;
	struct timeb now;
	rl = get_ratelimit(er);
...
const char *reader_get_type_desc(struct s_reader *rdr)
{
	...
	{
		if (cccam_client_multics_mode(rdr->client) && cccam_client_newbox_mode(rdr->client) && cccam_client_multics_hellboy_mode(rdr->client))
			desc = "cccam";
		else
		{
...
int32_t casc_process_ecm(struct s_reader *reader, ECM_REQUEST *er)
{
	...
	for(n = -1, i = 0, sflag = 1; i < cfg.max_pending; i++)
	{
		...
		// ecm already pending
		// ...this level at least
		if((ecm->rc >= E_NOCARD) && er->caid == ecm->caid && er->srvid == ecm->srvid && (!memcmp(er->ecmd5, ecm->ecmd5, CS_ECMSTORESIZE)))
			{ sflag = 0; }
