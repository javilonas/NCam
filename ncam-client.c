...
#endif
	case 'c':
		if (cccam_client_multics_mode(cl) && cccam_client_newbox_mode(cl) && cccam_client_multics_hellboy_mode(cl)) {
			ctyp = "cccam";
			break;
		}
...
/* Creates the master client of NCam and inits some global variables/mutexes. */
void init_first_client(void)
{
	// get username NCam is running under
	struct passwd pwd;
