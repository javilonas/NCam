...
// [zaplist] ACoSC anticascading
	...
	if(cfg.double_check && er->rc < E_NOTFOUND && er->selected_reader && is_double_check_caid(er) && (er->checked < 2))
	{
		if(er->checked == 0) // First CW, save it and wait for next one
		{
			er->checked = 1;
			er->origin_reader = er->selected_reader;
			memcpy(&er->cw_checked[8], &er->cw[4], 8);
			cs_debug_mask(D_TRACE, "DOUBLE CHECK FIRST CW by %s", er->origin_reader->label);
		}
		else if(er->origin_reader != er->selected_reader) // Second (or third and so on) cw. We have to compare
		{
			...
			{
				er->checked++;
				cs_debug_mask(D_TRACE, "DOUBLE CHECKED (from %s)", er->selected_reader->label);
			} else if (memcmp(er->cw_checked, &er->cw[4], 8) == 0) {
				er->checked++;
				cs_log("DOUBLE CHECK FIRST CW WAS INVALID (from %s, solved by %s)", er->origin_reader->label, er->selected_reader->label);
			} else {
				memcpy(er->cw_checked, &er->cw[4], 8);
				cs_log("DOUBLE CHECKED NONMATCHING (from %s, first %s)", er->selected_reader->label, er->origin_reader->label);
			}
		}
...
#ifdef CS_CACHEEX
	/* if answer from cacheex-1 reader, not send answer to client!
	 * thread check_cache will check counter and send answer to client!
...
static void logCWtoFile(ECM_REQUEST *er, uint8_t *cw)
{
	...
	*/
	...
	if(writeheader)
	{
		...
		fprintf(pfCWL, "# NCam cardserver v%s - https://www.lonasdigital.com\n", CS_VERSION);
...
int32_t write_ecm_answer(struct s_reader *reader, ECM_REQUEST *er, int8_t rc, uint8_t rcEx, uint8_t *cw, char *msglog, uint16_t used_cardtier, EXTENDED_CW* cw_ex)
{
	...
	if(cs_dblevel & D_TRACE)
	{
		...
		else if(ea->rc == E_NOTFOUND)
		{
			...
			{
				rdr_log(reader, "ECM not found limit reached %u. Restarting the reader.", reader->ecmsnok);
...
void get_cw(struct s_client *client, ECM_REQUEST *er)
{
	...
	if(er->ecmlen > 2)
	{
		...
		// Revert "Fix: valid section length is minimal 4"
