...
static void WriteKeyToFile(char identifier, uint32_t provider, const char *keyName, uint8_t *key, uint32_t keyLength, char* comment)
{
	...
	if(comment)
	{
		snprintf(line, sizeof(line), "\n%c %.8X %s %s ; added by NEmu %s %s\n", identifier, provider, keyName, keyValue, dateText, comment);
	}
	else
	{
		snprintf(line, sizeof(line), "\n%c %.4X %s %s ; added by NEmu %s\n", identifier, provider, keyName, keyValue, dateText);
	}
	cs_log("Key written: %c %.8X %s %s", identifier, provider, keyName, keyValue);
	free(keyValue);
					  ...
					  uint8_t *dw, uint16_t srvid, uint16_t ecmpid)
{
	...
	//try to get the pid from ncam's fake ecm ([sid] ([pid1] [pid2] ... [pidx])
	if(ecmDataLen >= 3) {
	...
	do 
	{
		...
		switch(nanoType)
		{
			case 0xED: // ECM_TAG_CW_DESCRIPTOR
			{
				if(nanoLength != 0x26)
				{
					cs_log("warning: ECM_TAG_DESCRIPTOR length (%d) != %d", nanoLength, 0x26);
					break;
				}
		...
		switch(tagType)
		{
			case 0xE4: // EMM_TAG_SECURITY_TABLE_DESCRIPTOR (ram emm keys)
			{
				...
				switch(tagMode)
				{
					...
					default:
						cs_log("warning: nanoTag E4 %.2X not supported", tagMode);
					break;
				}
				break;
			}
