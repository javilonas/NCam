...
int32_t dvbapi_net_send(uint32_t request, int32_t socket_fd, uint32_t msgid, int32_t demux_index, uint32_t filter_number, unsigned char *data, struct s_client *client, ECM_REQUEST *er, uint16_t client_proto_version)
{
	...
	switch (request)
	{
		case DVBAPI_SERVER_INFO:
		{
			...
			*info_len = snprintf((char *) &packet[size], sizeof(packet) - size, "NCAm v%s-%s, build: %s (%s); %s", CS_VERSION, CS_REVISION, DATE_BUILD, CS_TARGET, capabilities + 1);
			size += *info_len;
			break;
		}
	...
	while (i < BOX_COUNT)
	{
		do
		{
			...
			if(filtercount > 0)
			{
				...
				cs_log("Detected %s Api: %d, userconfig boxtype: %d maximum amount of possible filters is %d (ncam limit is %d filters and %d sidbits)",
					device_path, selected_api, cfg.dvbapi_boxtype, filtercount, MAX_FILTER, MAX_SIDBITS);
			}
...
int32_t dvbapi_stop_filternum(int32_t demux_index, int32_t num, uint32_t msgid)
{
	...
	if(demux[demux_index].demux_fd[num].type == TYPE_ECM)   //ecm filter stopped: reset index!
	{
		...
		// workaround: below dont run on stapi since it handles it own pids.... stapi need to be better integrated in ncam dvbapi.
		if(selected_api != STAPI)
		{
			int32_t z;
...
void dvbapi_add_ecmpid_int(int32_t demux_id, uint16_t caid, uint16_t ecmpid, uint32_t provid, uint32_t cadata, char *txt) 
{
	...
	if(caid_is_irdeto(caid) || (caid_is_dre(caid) && (provid == 0x11 || provid == 0xFE))) { demux[demux_id].emmstart.time = 1; }
...
void dvbapi_add_emmpid(int32_t demux_id, uint16_t caid, uint16_t emmpid, uint32_t provid, uint32_t cadata, uint8_t type)
{
	...
	if(cadata > 0) snprintf(cadatatext, 40, " CA DATA %X ", cadata);
	else {cadatatext[0] = '\t'; cadatatext[1] = '\0';}

	if(caid == 0x4AE1 && provid == 0x11 && cadata == 0) return;
...
ca_index_t dvbapi_get_descindex(int32_t demux_index, int32_t pid, int32_t stream_id)
{
	...
	{
		if(idx > INDEX_MAX_LOCAL)
		{
			idx = INDEX_INVALID;
		}
	}

	demux[demux_index].ECMpids[pid].index[stream_id] = idx;
...
void dvbapi_read_priority(void)
{
	...
	const char *cs_prio = "ncam.dvbapi";
	...
	while(fgets(token, sizeof(token), fp))
	{
					   ...
					   && type != 'j' && type != 'a' && type != 'x'))
		{
			...
			//however it takes NCAm a long time (>4 seconds) to reach this part of the program, so the warnings are reaching tty rather late
		...
		{
			ret = fclose(fp);
			if(ret < 0) { cs_log("ERROR: Could not close ncam.dvbapi fd (errno=%d %s)", errno, strerror(errno)); }
			return;
		}
...
//		entry->last=NULL;
		...
		if(ret < 1)
		{
			cs_log("Error in ncam.dvbapi: ret=%d | %c: %04X %06X %s %04X %04X",
				...
				for(this = cfg.srvid[i]; this != NULL; this = this->next)
				{
					if(this->name && strcmp(this->name, c_srvid + 1) == 0)
					{
						...
						entry2->srvid = this->srvid;
						 ...
						 cs_log_dbg(D_DVBAPI, "prio srvid: ret=%d | %c: %04X %06X %04X %04X %04X -> map %04X %06X %04X | prio %d | delay %d", ret, entry2->type, entry2->caid, entry2->provid, entry2->srvid, entry2->ecmpid, entry2->chid, entry2->mapcaid, entry2->mapprovid, entry2->mapecmpid, entry2->force, entry2->delay);
		...
		cs_log_dbg(D_DVBAPI, "prio: ret=%d | %c: %04X %06X %04X %04X %04X -> map %04X %06X %04X | prio %d | delay %d", ret, entry->type, entry->caid, entry->provid, entry->srvid, entry->ecmpid, entry->chid, entry->mapcaid, entry->mapprovid, entry->mapecmpid, entry->force, entry->delay);
...
void dvbapi_resort_ecmpids(int32_t demux_index)
{
	...
	// handle prio order in ncam.dvbapi + ignore all chids
	...
	for(prio = dvbapi_priority; prio != NULL; prio = prio->next)
	{
		...
		for(n = 0; n < demux[demux_index].ECMpidcount; n++)
		{
			...
			{
				if(prio->cadata != 0 && prio->cadata != demux[demux_index].ECMpids[n].cadata) { continue; }
				...
				{
					if(!demux[demux_index].ECMpids[n].status) // only accept first matching prio from ncam.dvbapi
					{
	...
	// ecmpid with highest prio from ncam.dvbapi has now highest status
	...
	for(j = status; j >= 0; j--)    // largest status first!
	{
		...
		for(n = 0; n < demux[demux_id].ECMpidcount; n++)
		{
			...
			if(demux[demux_id].ECMpids[n].checked == checked && demux[demux_id].ECMpids[n].status == j)
			{
				...
				if(caid_is_irdeto(demux[demux_id].ECMpids[found].CAID) 
...
#define LIST_UPDATE 0x05 //*CA application should replace an entry in the list with an 'UPDATE' CAPMT object, and start working with the updated list
	...
	char channame[CS_SERVICENAME_SIZE];
	get_servicename(dvbapi_client, demux[demux_id].program_number, demux[demux_id].ECMpidcount > 0 ? demux[demux_id].ECMpids[0].PROVID : NO_PROVID_VALUE, demux[demux_id].ECMpidcount > 0 ? demux[demux_id].ECMpids[0].CAID : NO_CAID_VALUE, channame, sizeof(channame));
	cs_log_dbg(D_DVBAPI,"Demuxer %d serving srvid %04X (%s) on adapter %04X camask %04X index %04X pmtpid %04X", demux_id, demux[demux_id].program_number, channame, adapter_index, ca_mask, demux_index, pmtpid);
...
static uint32_t dvbapi_extract_sdt_string(char *buf, uint32_t buflen, uint8_t* source, uint32_t sourcelen)
{
	...
	if(iso_mode >= -1)
	{
		...
		cs_log_dbg(D_DVBAPI, "sdt-info warning: your build of ncam does not support iso-to-utf8 conversion, special chars may be corrupted!");
	}
#endif
	...
	for(pos = 11; pos+5 < length; pos += descriptor_length)
	{
		...
		for(dpos = 0; dpos+1 < descriptor_length; dpos += (2 + data_length))
		{
			...
			if(strlen(provider_name) && caid != NO_CAID_VALUE)
			{
				...
				if(tmp[0] == '\0')
				{
					get_config_filename(tmp, sizeof(tmp), "ncam.provid");
			...
			if(strlen(service_name))
			{
				...
				if(tmp[0] == '\0')
				{
					...
					get_config_filename(tmp, sizeof(tmp), "ncam.srvid2");
					...
					{
						get_config_filename(tmp, sizeof(tmp), "ncam.srvid");
...
int32_t dvbapi_init_listenfd(void)
{
	...
	// this will allow ncam to run as root if needed
...
void dvbapi_process_input(int32_t demux_id, int32_t filter_num, uchar *buffer, int32_t len, uint32_t msgid)
{
	...
	if(filtertype == TYPE_ECM)
	{
		...
		if(len != 0)  // len = 0 receiver encountered an internal bufferoverflow!
		{
			...
			{
				cs_log_dbg(D_DVBAPI, "Received data with total length 0x%03X but maximum ECM length ncam can handle is 0x%03X -> Please report!", sctlen, MAX_ECM_SIZE);
				if(curpid)
				{
					curpid->tries-=0x0E;
				}
...
static uint16_t dvbapi_get_nbof_missing_header_bytes(uchar* mbuf, uint16_t mbuf_len)
{
	if(mbuf_len < 4)
	{
		return 4 - mbuf_len;
									...
									uint8_t* add_to_poll, uint16_t* new_unhandled_len, uint16_t* client_proto_version)
{
	...
	if(missing_header_bytes)
	{
		...
		cs_log_dbg(D_TRACE, "%s to read %" PRIu16 " bytes from connection fd %d", (unhandled_len == 0) ? "Trying":"Continue", missing_header_bytes, connfd);
...
#define PMT_SERVER_SOCKET "/tmp/.listen.camd.socket"
	...
	struct SOCKADDR servaddr;
	ssize_t len = 0;
	static const uint16_t mbuf_size = 2048*2;
	uchar *mbuf;
	...
	while(!exit_ncam)
	{
		...
		if(rc > 0)
		{
			...
			if (timeout < 0) {
				cs_log("*** WARNING: BAD TIME AFFECTING WHOLE NCAM ECM HANDLING ****");
			}
					...
					if (connfd > 0) {
						...
						if(unhandled_buf_used[i])
						{
							if(unhandled_buf_used[i] > unhandled_buf_len[i])
							{
								...
								if(!cs_malloc(&unhandled_buf[i], sizeof(uchar)*unhandled_buf_len[i]))
								{
									unhandled_buf_len[i] = 0;
									unhandled_buf_used[i] = 0;
									continue;
...
#define ECMINFO_TYPE_CAMD3		5
	...
	if(ecmtxt != NULL && er->rc < E_NOTFOUND)
	{
		...
		if(cfg.dvbapi_ecminfo_type <= ECMINFO_TYPE_NCAM_MS)
		{
			switch(er->rc)
			{
			case E_FOUND:
			...
			if(cfg.dvbapi_ecminfo_type == ECMINFO_TYPE_NCAM)
...
ca_index_t dvbapi_ca_setpid(int32_t demux_index, int32_t pid, int32_t stream_id, bool use_des, uint32_t msgid)
{
	...
	if(demux[demux_index].ECMpids[pid].useMultipleIndices)
	{
		...
		{
			idx = dvbapi_get_descindex(demux_index, pid, n);
			if(idx == INDEX_INVALID)
			{
				cs_log_dbg(D_DVBAPI, "Demuxer %d PID: %d CAID: %04X ECMPID: %04X has no free index", demux_index, pid,
					demux[demux_index].ECMpids[pid].CAID, demux[demux_index].ECMpids[pid].ECM_PID);
	...
	else
	{
		...
		if(idx == INDEX_INVALID)   // if no indexer for this pid get one!
		{
			idx = dvbapi_get_descindex(demux_index, pid, 0);
			if(idx == INDEX_INVALID)
			{
				cs_log_dbg(D_DVBAPI, "Demuxer %d PID: %d CAID: %04X ECMPID: %04X has no free index", demux_index, pid,
					demux[demux_index].ECMpids[pid].CAID, demux[demux_index].ECMpids[pid].ECM_PID);
...
ca_index_t is_ca_used(uint8_t cadevice, int32_t pid)
{
	...
	if(ll_count(ll_activestreampids) > 0)
	{
		...
		while((listitem = ll_iter_next(&itr)))
		{
			if(listitem->cadevice != cadevice) 
				{ continue; }

			if(pid && listitem->streampid != pid) 
				{ continue; }
