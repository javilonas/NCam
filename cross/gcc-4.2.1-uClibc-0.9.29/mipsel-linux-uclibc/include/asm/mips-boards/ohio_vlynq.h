/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
#ifndef _ohio_vlynq_h_
#define _ohio_vlynq_h_

/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
#include <linux/interrupt.h>
#include <asm/mach-ohio/hw_vlynq.h>
...
#define OHIO_VLYNQ_INIT_RETRY   25

#define OHIO_VLYNQ_INT_SHIFT        7
#define OHIO_VLYNQ_TYPE_SHIFT       6
#define OHIO_VLYNQ_POLARITY_SHIFT   5
...
struct _vlynq_interrupts {
    ...
    unsigned int irq;   /*--- wird von ohio_vlynq_init_link() gefuellt ---*/
};
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
unsigned int ohio_vlynq_init(void);
/*unsigned int ohio_vlynq_init_link(unsigned int instance, struct _vlynq_context *context);*/
unsigned int ohio_vlynq_init_link(unsigned int instance);

struct _vlynq_context *ohio_vlynq_alloc_context(unsigned int instance);
unsigned int ohio_vlynq_free_context(unsigned int instance);

irqreturn_t vlynq_interrupt(int irq, void *dev_id, struct pt_regs *regs);

#endif /*--- #ifndef _ohio_vlynq_h_ ---*/
