...
void init_cw_cache(void){
	if(cfg.cw_cache_size > 0 || cfg.cw_cache_memory > 0){
		tommy_hashlin_init(&ht_cw_cache);
...
{
	if (er->ecm[0] != 0x80 && er->ecm[0] != 0x81 &&	!(caid_is_dvn(er->caid) && er->ecm[0] == 0x50)) {
		return 0;
	}
	return er->ecm[0];
}
...
void cw_cache_cleanup(bool justfree){
	if(!cw_cache_init_done)
		{ return; }

	//free & done
	tommy_hashlin_foreach(&ht_cw_cache, free);
	tommy_hashlin_done(&ht_cw_cache);
	if(!justfree)
		tommy_hashlin_init(&ht_cw_cache);
}
		...
		if(cw_cache_setting.mode > 0){
			...
			if(!cw_cache ){
				...
				if(
					...
					|| (cfg.cw_cache_memory && (cfg.cw_cache_memory*1024*1024 > tommy_hashlin_memory_usage(&ht_cw_cache)))
				){
					...
					// clean cache call;
					cs_log_dbg(D_CW_CACHE,"[cw_cache] cw_cache full; cw_cache_size: %u => cleanup => kill&init again....", cfg.cw_cache_size);
			...
			// cw found
				...
				// same cw for different srvid incoming WTF? => drop
					...
					cs_log_dbg(D_CW_CACHE,"[cw_cache][dupe CW][srvid diff] cache: %04X:%06X:%04X:%s | in: %04X:%06X:%04X:%s | diff(now): %ums", cw_cache->caid, cw_cache->prid, cw_cache->srvid, cw1, er->caid, er->prid, er->srvid, cw2, gone_diff);
					drop_cw = 1;
				}

				if(cw_cache_setting.mode > 1 && drop_cw){
					cs_log_dbg(D_CW_CACHE,"[cw_cache] incoming CW dropped - cw_cache_size: %i - current cw_cache-mem-size: %lub", count_hash_table(&ht_cw_cache), tommy_hashlin_memory_usage(&ht_cw_cache));
					SAFE_RWLOCK_UNLOCK(&cache_lock);
					return;
				}
			}
