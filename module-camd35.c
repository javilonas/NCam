...
static void camd35_request_emm(ECM_REQUEST *er)
{
	...
	if(!au_caid && (caid_is_bulcrypt(er->caid) || caid_is_streamguard(er->caid) || caid_is_tongfang(er->caid))) // Bulcrypt/StreamGuard has 2 caids and aureader->caid can't be used. Use ECM_REQUEST caid for AU.
		{ au_caid = er->caid; }
