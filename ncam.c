...
static void show_usage(void)
{
	...
	printf("\n");
	printf("Copyright (C) 2012-2016 developed by Javilonas.\n");
	...
	printf("\n Logging:\n");
	printf(" -I, --syslog-ident <ident> | Set syslog ident. Default: ncam\n");
...
static void write_versionfile(bool use_stdout)
{
	...
	{
		char targetfile[256];
		fp = fopen(get_tmp_dir_filename(targetfile, sizeof(targetfile), "ncam.version"), "w");
		if(!fp)
		{
...
{
	char targetfile[256];
	unlink(get_tmp_dir_filename(targetfile, sizeof(targetfile), "ncam.version"));
}
...
/**
 * write stacktrace to ncam.crash. file is always appended
 * Usage:
 * 1. compile ncam with debug parameters (Makefile: DS_OPTS="-ggdb")
 * 2. you need gdb installed and working on the local machine
 * 3. start ncam with parameter: -a
 */
static void cs_dumpstack(int32_t sig)
{
	FILE *fp = fopen("ncam.crash", "a+");
 ...
 *
 * reloads configs:
 *  - useraccounts (ncam.user)
 *  - readers      (ncam.server)
 *  - services ids (ncam.srvid)
 *  - tier ids     (ncam.tiers)
...
// ncam without SEQFAULT (stack overflow), this is risky business.
// If after a code change SEQFAULTs related to stack overflow appear,
// increase NCAM_STACK_MIN or remove the calls to SAFE_ATTR_SETSTACKSIZE.
...
static void process_clients(void)
{
	...
	while(!exit_ncam)
	{
		...
		for(i = 0; i < pfdcount && rc > 0; i++)
		{
			...
			rc--; //event handled!
			cs_log_dbg(D_TRACE, "[NCAM] new event %d occurred on fd %d after %"PRId64" ms inactivity", pfd[i].revents,
			...
			if(pfd[i].fd == thread_pipe[0] && (pfd[i].revents & (POLLIN | POLLPRI)))
			{
				...
				if(len == -1)
				{
					cs_log_dbg(D_TRACE, "[NCAM] Reading from pipe failed (errno=%d %s)", errno, strerror(errno));
				}
				cs_log_dump_dbg(D_TRACE, buf, len, "[NCAM] Readed:");
				continue;
			}
			...
			if(rdr && cl2 && cl2->init_done)
			{
				if(cl2->pfd && pfd[i].fd == cl2->pfd && (pfd[i].revents & (POLLHUP | POLLNVAL | POLLERR)))
				{
					...
					//ncam should check for rdr->tcp_connected and reconnect on next ecm request sent to the proxy
...
int32_t main(int32_t argc, char *argv[])
{
	...
	if(!ncam_pidfile && cfg.pidfile)
		{ ncam_pidfile = cfg.pidfile; }
	if(!ncam_pidfile)
	{
		ncam_pidfile = get_tmp_dir_filename(default_pidfile, sizeof(default_pidfile), "ncam.pid");
	}
	if(ncam_pidfile)
		{ pidfile_create(ncam_pidfile); }
	cs_init_statistics();
	coolapi_open_all();
	init_stat();
	ssl_init();
