...
static void camd35_request_emm(ECM_REQUEST *er)
{
	...
	if(!au_caid && (caid_is_bulcrypt(er->caid) || caid_is_streamguard(er->caid) || caid_is_tongfang(er->caid) || caid_is_dvn(er->caid))) // Bulcrypt/StreamGuard has 2 caids and aureader->caid can't be used. Use ECM_REQUEST caid for AU.
		{ au_caid = er->caid; }
...
static void camd35_send_dcw(struct s_client *client, ECM_REQUEST *er)
{
	...
	else
	{
		...
		{
			// Send old CMD44 to prevent cascading problems with older mpcs/ncam versions
			buf[0] = 0x44;
			buf[1] = 0;
		}
	}
