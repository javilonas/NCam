...
int32_t has_lb_prio_srvid(struct s_reader *rdr, ECM_REQUEST *er)
{
	...
	for(nr = 0, sidtab = cfg.sidtab; sidtab; sidtab = sidtab->next, nr++)
		if((rdr->lb_prio_sidtabs.ok & ((SIDTABBITS)1 << nr)) &&
				(chk_srvid_match(er, sidtab)))
			{ return 1; }
	return 0;
}
...
{
	if( caid_is_videoguard(er->caid) && (er->caid == 0x09C4 || er->caid == 0x098C || er->caid == 0x098D || er->caid == 0x0960 || er->caid == 0x0963 || er->caid == 0x09CD || er->caid == 0x0919 || er->caid == 0x093B || er->caid == 0x098E))
		{ return 1; }
	return 0;
}
