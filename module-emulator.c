 ...
 * order to be compatible with NCam's reader structure.
*/
 ...
 * for every cryptosystem supported by NCam.
 * Here we read keys from our virtual card (aka the SoftCam.Key file) and we inform
 * NCam about them. This is done with the emu_card_info() function. Keep in mind
...
static int32_t emu_card_info(struct s_reader *rdr)
{
	...
	// Read keys built in the NCam-Emu binary
...
int32_t emu_get_biss_emm_type(EMM_PACKET *ep, struct s_reader *rdr)
{
	switch (ep->emm[0])
	{
		case 0x81: // Spec say this is for EMM, but ncam (and all other crypto systems) use it for ECM
		case 0x82:
		case 0x83:
		case 0x84:
		case 0x85:
...
{
	// This function is called inside ncam.c and creates an emu [reader] with default
	// settings in ncam.server file. If an emu [reader] already exists, it uses that.
	...
	// Check if emu [reader] entry already exists in ncam.server file and get it
	...
	// If there's no emu [reader] in ncam.server, create one with default settings
	if (!haveEmuReader)
	{
	...
	cs_log("NCam-Emu version %d", EMU_VERSION);
}
