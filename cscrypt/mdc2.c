...
#if !defined(WITH_LIBCRYPTO) || WITH_LIBCRYPTO == 0 || !OPENSSL_MIN_API || defined(OPENSSL_NO_DES)
//OPENSSL_GLOBAL const DES_LONG DES_SPtransNEMU[8][64] =
const DES_LONG DES_SPtransNEMU[8][64] =
{
	{
		/* nibble 0 */
...
# define D_ENCRYPT(LL,R,S) { \
        ...
        t=ROTATE(t,4); \
        LL^= \
            DES_SPtransNEMU[0][(u>> 2L)&0x3f]^ \
            DES_SPtransNEMU[2][(u>>10L)&0x3f]^ \
            DES_SPtransNEMU[4][(u>>18L)&0x3f]^ \
            DES_SPtransNEMU[6][(u>>26L)&0x3f]^ \
            DES_SPtransNEMU[1][(t>> 2L)&0x3f]^ \
            DES_SPtransNEMU[3][(t>>10L)&0x3f]^ \
            DES_SPtransNEMU[5][(t>>18L)&0x3f]^ \
            DES_SPtransNEMU[7][(t>>26L)&0x3f]; }
...
#if !defined(WITH_LIBCRYPTO) || WITH_LIBCRYPTO == 0 || !OPENSSL_MIN_API || defined(OPENSSL_NO_DES)
void DES_encrypt1NEMU(DES_LONG *data, DES_key_schedule *ks, int enc)
{
	 ...
	 * the loop.  This required the DES_SPtransNEMU values in sp.h to be rotated
...
static void mdc2_body(MDC2_CTX *c, const unsigned char *in, size_t len)
{
	...
	for (i = 0; i < len; i += 8)
	{
		...
		DES_encrypt1NEMU(d, &k, 1);
		DES_encrypt1NEMU(dd, &k, 1);
